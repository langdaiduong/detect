...
def draw_bbox(image, bboxes, classes = read_class_names(cfg.YOLO.CLASSES), allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), show_label=True):
    ...
    for i in range(num_classes):
        ...
        ##############################################
        # xử lý dộ chính xác của box cao nhất
        ...
        if class_name not in allowed_classes:
            ...
            ########################################################################
            # xử lý chỉ vẽ box có độ chính xác cao nhẩt
            if show_label and score > 0.5:
                if score == max_score[i]:
                    ...
                    cv2.putText(image, bbox_mess, (c1[0], np.int(c1[1] - 2)), cv2.FONT_HERSHEY_SIMPLEX,
